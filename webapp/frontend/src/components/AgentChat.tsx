...
export const AgentChat: React.FC<AgentChatProps> = ({ agentName, agentDescription, agentLogo, starterPrompts, userDisplayName, onSignOut }) => {
  ...
  // The sidebar is a persistent docked panel now, visible by default — load its contents once
  // on mount instead of only lazily when a (now-removed) open toggle fired.
  const hasLoadedConversationsRef = useRef(false);
  useEffect(() => {
    if (state.conversations.sidebarOpen && !hasLoadedConversationsRef.current) {
      hasLoadedConversationsRef.current = true;
      loadConversations();
    }
  }, [state.conversations.sidebarOpen, loadConversations]);

  const handleToggleSidebar = useCallback(async () => {
    const willOpen = !state.conversations.sidebarOpen;
    dispatch({ type: 'CONVERSATIONS_TOGGLE_SIDEBAR' });
    if (willOpen && !hasLoadedConversationsRef.current) {
      hasLoadedConversationsRef.current = true;
      await loadConversations();
    }
  }, [state.conversations.sidebarOpen, dispatch, loadConversations]);
      ...
      <ConversationSidebar
        ...
        footer={
          userDisplayName && onSignOut ? (
            <UserMenu displayName={userDisplayName} onSignOut={onSignOut} />
          ) : undefined
        }
      />
