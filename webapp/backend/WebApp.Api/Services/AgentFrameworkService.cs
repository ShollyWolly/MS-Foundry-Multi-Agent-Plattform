...
public class AgentFrameworkService : IDisposable
{
    ...
    /// <summary>The logged-in user's identity, used to scope conversations in <see cref="_conversationStore"/>.</summary>
        ...
        IHttpContextAccessor? httpContextAccessor = null)
    {
        ...
        _fallbackCredential = AzureCredentialResolver.Resolve(configuration, _logger, _managedIdentityClientId);
    ...
    public async Task<string> CreateConversationAsync(string? firstMessage = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await _conversationStore.RegisterAsync(conversation.Id, CurrentUserId, title, cancellationToken);
    ...
    /// <summary>
    /// List conversations owned by the current user (Cosmos-backed index — Foundry's own
    /// conversation store is agent-scoped, not user-scoped, so it can't answer "my conversations").
    /// </summary>
    public async Task<List<ConversationSummary>> ListConversationsAsync(int limit = 20, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            _logger.LogInformation("Listing conversations for user {UserId} (limit={Limit})", CurrentUserId, limit);

            var records = await _conversationStore.ListAsync(CurrentUserId, limit, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (await _conversationStore.GetAsync(conversationId, CurrentUserId, cancellationToken) is null)
            {
                throw new UnauthorizedAccessException(
                    $"Conversation '{conversationId}' does not exist or is not owned by the current user.");
            }
    ...
    /// </summary>
    /// <remarks>
    /// The Azure.AI.Projects SDK still doesn't expose a delete on the Foundry conversation
    /// itself (ProjectConversationsClient only supports Create/Get/List/Update), but since
    /// conversation *ownership/visibility* is now tracked in Cosmos (<see cref="_conversationStore"/>),
    /// a soft-delete there is a real, working delete from the user's point of view — it just
    /// leaves the underlying Foundry conversation (and its message content) in place.
    /// </remarks>
    public async Task DeleteConversationAsync(string conversationId, CancellationToken cancellationToken = default)
    {
        ObjectDisposedException.ThrowIf(_disposed, this);

        if (!await _conversationStore.SoftDeleteAsync(conversationId, CurrentUserId, cancellationToken))
        {
            throw new UnauthorizedAccessException(
                $"Conversation '{conversationId}' does not exist or is not owned by the current user.");
        }
