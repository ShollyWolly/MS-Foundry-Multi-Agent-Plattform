...
// AUTH_MODE switch: "entra" (default) is the original, unmodified MSAL/JWT-bearer flow below.
// "mock" is an additive local-dev branch for the Docker stack, used while the deployment's
// service principal has no Entra ID app-registration permissions to provision the SPA app
// registration the real flow needs. Flipping AUTH_MODE back to "entra" (or unsetting it)
// restores the original behavior untouched — nothing in the entra branch is deleted or altered.
var authMode = builder.Configuration["AUTH_MODE"] ?? "entra";

if (authMode == "mock")
{
    builder.Services.AddAuthentication("MockScheme")
        .AddCookie("MockScheme", options =>
        {
            options.Cookie.Name = "mock_auth";
    ...
    CancellationToken cancellationToken) =>
{
    ...
    {
        // Fail closed without confirming whether the conversation exists for another user.
...
// Mock-auth endpoints — only mapped when AUTH_MODE=mock (see the switch above). Leaves the
// real Entra flow's endpoints/middleware completely untouched when not in mock mode.
if (authMode == "mock")
{
    app.MapPost("/api/auth/login", async (LoginRequest request, HttpContext ctx, IConfiguration config) =>
    {
        var expectedUsername = config["MOCK_AUTH_USERNAME"];
        var expectedPassword = config["MOCK_AUTH_PASSWORD"];
        if (string.IsNullOrEmpty(expectedUsername) || string.IsNullOrEmpty(expectedPassword)
            || request.Username != expectedUsername || request.Password != expectedPassword)
        {
            return Results.Unauthorized();
        }

        var identity = new ClaimsIdentity(new[] { new Claim(ClaimTypes.Name, request.Username) }, "MockScheme");
        await ctx.SignInAsync("MockScheme", new ClaimsPrincipal(identity));
    ...
    {
        await ctx.SignOutAsync("MockScheme");
        return Results.Ok();
    })
    .AllowAnonymous()
    .WithName("MockLogout");
